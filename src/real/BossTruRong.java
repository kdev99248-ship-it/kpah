...
public class BossTruRong extends Boss {
    byte wave = 0;
    ...
    public BossTruRong(Map mapLiveIn, MonsterTemplate template, int x, int y, int country) {
    ...
    public void update() {
        ...
        if (!Map.getTown[this.inCountry]) {
            this.actorDie();
            this.map.removeDynamicMonster(this, this.inCountry, 0);
        } else if (!this.isDead && this.hp > 0) {
            NpcReceiveCard npc1 = (NpcReceiveCard)((Vector)Map.npcReceiveCard.get(this.inCountry)).get(this.posTower);
